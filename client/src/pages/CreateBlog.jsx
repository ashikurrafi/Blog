...
//       formDataToSend.append("imageBlog", formData.imageBlog);
...
//       toast.error("Title and content are required to save draft");
//       return;
//     }
...
const CreateBlog = () => {
  ...
  const categories = [
    ...
    "DevOps", "UI/UX Design", "Business", "Lifestyle", "Travel", "Food", "Health",
    "Education", "Entertainment", "Sports", "News", "Other",
  ];
  ...
  const handleSubmit = async (e) => {
    ...
    if (isPublished && !imageBlog) return toast.error("Image required for publishing");
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 dark:from-gray-900 dark:to-gray-800 p-4 md:p-6">
      <div className="max-w-4xl mx-auto space-y-6">
        <div className="text-center space-y-2">
          <h1 className="text-3xl md:text-4xl font-bold text-gray-900 dark:text-white">Create New Blog</h1>
          <p className="text-gray-600 dark:text-gray-300">Share your thoughts and ideas with the world</p>
        </div>

        <Card className="shadow-xl border-0 bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm">
          <CardHeader>
            <CardTitle className="flex items-center gap-2"><FileText className="w-5 h-5" /> Blog Details</CardTitle>
          ...
          <CardContent>
            <form onSubmit={handleSubmit} className="space-y-6">
              {/* Title */}
              <div className="space-y-2">
                <Label htmlFor="title" className="flex items-center gap-2"><Type className="w-4 h-4" /> Title</Label>
                <Input
                  id="title"
                  name="title"
                  value={formData.title}
              ...
              {/* Image Upload */}
              <div className="space-y-2">
                <Label className="flex items-center gap-2"><ImageIcon className="w-4 h-4" /> Blog Image</Label>
                {!imagePreview ? (
                  <div className="border-2 border-dashed rounded-lg p-8 text-center">
                    <div className="space-y-4">
                      ...
                      <Label htmlFor="imageBlog" className="cursor-pointer text-blue-600 hover:underline">Click to upload an image</Label>
                      <p className="text-sm text-gray-500">PNG, JPG, GIF up to 5MB</p>
                      <Input id="imageBlog" type="file" accept="image/*" onChange={handleImageChange} className="hidden" />
                    </div>
                  </div>
                ) : (
                  <div className="relative">
              ...
              {/* Content */}
              <div className="space-y-2">
                <Label htmlFor="content" className="flex items-center gap-2"><FileText className="w-4 h-4" /> Content</Label>
                <Textarea
                  id="content"
                  name="content"
                  value={formData.content}
              ...
              <div className="flex items-center space-x-2">
                ...
                />
                <Label htmlFor="isPublished" className="flex items-center gap-2">
                  {formData.isPublished ? <Eye className="w-4 h-4 text-green-600" /> : <EyeOff className="w-4 h-4 text-gray-400" />}
                  Publish immediately
                </Label>
              </div>
              ...
              <div className="flex flex-wrap gap-3 pt-4">
                ...
                <Button type="button" variant="outline" onClick={loadDraft} className="gap-2">
                  Load Draft
                </Button>
                ...
                <Dialog open={showPreviewDialog} onOpenChange={setShowPreviewDialog}>
                  ...
                  <DialogContent className="max-w-4xl max-h-[80vh] overflow-y-auto">
                    <DialogHeader>
                      <DialogTitle>Blog Preview</DialogTitle>
                      <DialogDescription>This is how your blog will appear to readers</DialogDescription>
                    </DialogHeader>
                    <div className="space-y-4">
                      {imagePreview && (
                        <img src={imagePreview} alt="Preview" className="w-full h-64 object-cover rounded-lg" />
                      ...
                      <div className="space-y-2">
                        <h1 className="text-2xl font-bold">{formData.title || "Your Blog Title"}</h1>
                        <div className="flex items-center gap-4 text-sm text-gray-500">
                          <span className="flex items-center gap-1"><Calendar className="w-4 h-4" /> {new Date().toLocaleDateString()}</span>
                          <span className="flex items-center gap-1"><Tag className="w-4 h-4" /> {formData.category || "No category"}</span>
                          <span className="flex items-center gap-1">By {user?.user?.name || "Author"}</span>
                        </div>
                      </div>
                      <div className="prose max-w-none">{formData.content ? formatPreviewContent(formData.content) : <p className="text-gray-500 italic">Your blog content will appear here...</p>}</div>
                    </div>
                    <DialogFooter>
                      <DialogClose asChild>
                        <Button variant="outline">Close Preview</Button>
        ...
        {/* Writing Tips */}
        <Card className="shadow-lg border-0 bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm">
          <CardHeader><CardTitle className="text-lg">Writing Tips</CardTitle></CardHeader>
          <CardContent>
            <div className="grid md:grid-cols-2 gap-4 text-sm">
              <div>
                <h4 className="font-medium mb-2">📝 Content Tips:</h4>
